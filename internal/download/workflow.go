...
var (
	...
	downloadEpisodeRange       = api.DownloadEpisodeRangeEnhanced
	handleBatchDownloadRange   = func(episodes []models.Episode, anime *models.Anime, startNum, endNum int) error {
		return player.HandleBatchDownloadRange(episodes, anime, startNum, endNum)
	}
	setAnimeName             = player.SetAnimeName
	setExactMediaType        = player.SetExactMediaType
...
func HandleDownloadRequest(request *util.DownloadRequest) error {
	...
	if anime.IsMovie() {
		...
		return handleMovieDownloadFlow(movieRequest)
	}
	...
	if request.IsRange {
		...
		if request.AllAnimeSmart && (anime.Source == "AllAnime" || source == "allanime" || source == "AllAnime") {
			...
			if err := downloadAllAnimeSmartRange(anime, request.StartEpisode, request.EndEpisode, quality); err != nil {
				...
				if err := downloadEpisodeRange(anime, request.StartEpisode, request.EndEpisode, quality); err != nil {
					...
					dl := newEpisodeDownloader(episodes, anime.URL, anime)
...
func HandleMovieDownloadRequest(request *util.DownloadRequest) error {
	...
	selectedMedia, err := selectMovieFromResultsFn(results, request.IsMovie, request.IsTV)
	if err != nil {
	...
	setAnimeName(anime.Name, request.SeasonNum)

	// Create movie downloader
	md := newMovieDownloader(downloader.MovieDownloadConfig{
	...
	if request.IsMovie || selectedMedia.Type == scraper.MediaTypeMovie {
		...
		if seasonNum == 0 {
			...
			seasonNum, err = selectSeasonFn(mediaManager, extractIDFromURL(selectedMedia.URL))
			if err != nil {
		...
		if request.IsRange {
			...
			if episodeNum == 0 {
				...
				episodeNum, err = selectEpisodeFn(mediaManager, extractIDFromURL(selectedMedia.URL), seasonNum)
				if err != nil {
