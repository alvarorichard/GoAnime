...
var (
	...
	reSourceTag = regexp.MustCompile(`(?i)[🔥🌐]?\[(?:animefire|allanime|hianime|anidb|animedrive|9anime)\]\s*`)
	...
	reSpaceDashNoise = regexp.MustCompile(`(?i)\s+-\s+(?:` +
		...
		`parte\s*\d+|part\s*\d+|` +
		`allanime|hianime|anidb|animefire|animedrive|9anime|goyabu|superflix|flixhq|sflix` +
		`).*$`)
