...
func (e *Enricher) buildSeasonMapFromSuperFlix(ctx context.Context, animeName string) []SeasonMapping {
	...
	// Step 1: Search SuperFlix. Use the canonical .cyou host directly: legacy
	// hosts (.rest, .online, .best, .fit) 301-redirect here and Go's http.Client
	// downgrades POSTs to GETs across the redirect, which breaks the player API.
	searchURL := "https://superflixapi.cyou/pesquisar?s=" + url.QueryEscape(cleanName)
	...
	epURL := "https://superflixapi.cyou/serie/" + tmdbID
	...
	req2.Header.Set("Referer", "https://superflixapi.cyou/")
