package main

import (
	"encoding/base64"
	"fmt"
	"strconv"
)

func decodeHexToASCII(encoded string) string {
	decoded := ""

	// Split the encoded string into pairs of characters
	for i := 0; i < len(encoded); i += 2 {
		if i+1 >= len(encoded) {
			break
		}

		hexPair := encoded[i : i+2]

		// Convert hex pair to integer
		if val, err := strconv.ParseInt(hexPair, 16, 64); err == nil {
			decoded += string(rune(val))
		} else {
			decoded += hexPair // Keep as is if conversion fails
		}
	}

	return decoded
}

func main() {
	// Test with the encoded URL
	encoded := ""

	fmt.Printf("Step 1 - Hex to ASCII decoding:\n")
	asciiDecoded := decodeHexToASCII(encoded)
	fmt.Printf("ASCII decoded length: %d\n", len(asciiDecoded))

	// Let's try to clean up the string by removing non-printable characters and see if we get Base64
	cleaned := ""
	for _, r := range asciiDecoded {
		if r >= 32 && r <= 126 { // Printable ASCII characters
			cleaned += string(r)
		}
	}

	fmt.Printf("Cleaned ASCII (printable only): %s\n", cleaned)
	fmt.Printf("Cleaned length: %d\n", len(cleaned))

	// Try Base64 decoding
	if base64Decoded, err := base64.StdEncoding.DecodeString(cleaned); err == nil {
		fmt.Printf("Base64 decoded: %s\n", string(base64Decoded))
	} else {
		fmt.Printf("Base64 decode failed: %v\n", err)

		// Try URL-safe Base64
		if base64Decoded, err := base64.URLEncoding.DecodeString(cleaned); err == nil {
			fmt.Printf("URL-safe Base64 decoded: %s\n", string(base64Decoded))
		} else {
			fmt.Printf("URL-safe Base64 decode also failed: %v\n", err)
		}
	}

	// Let's also check if there's a pattern in the raw ASCII
	fmt.Printf("\nRaw ASCII with escape sequences:\n")
	fmt.Printf("%q\n", asciiDecoded)
}
